...
package body Textures_Manager is
    ...
    begin
        for Index in Images'Range loop
            Put_Line ("Load_Textures; loading image " & Integer'Image (Index) &
                        ": " & To_String (Images (Index)));
            GL.Images.Load_File_To_Texture
              (Path           => To_String (Images (Index)),
               Texture        => aTexture,
               Texture_Format => GL.Pixels.RGBA,
               Try_TGA        => True);
            Put_Line ("Load_Textures; image " & To_String (Images (Index)) &
                        " loaded.");
        end loop;
    ...
    exception
        when anError : others =>
            Put_Line ("An exception occurred in Load_Textures.");
