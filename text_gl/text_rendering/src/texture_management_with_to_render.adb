...
package body Texture_Management is
   ...
   begin
      ...
      if GL.Text.Created (Shader_Ref) then
        ...
        "Texture_Management.Initialize_Font_Data, Shader_Ref creation failed.";
      end if;
      return Renderer_Ref;
...
--                                      Text_Length * 3 * Triangles_Per_Quad;
   ...
   begin
      ...
      GL.Uniforms.Set_Int (Texture_ID, 1);
...
--        GL.Blending.Set_Blend_Func (Src_Alpha_Blend, One_Minus_Src_Alpha_Blend);
   ...
   begin
      if not Renderer_Ref.Created then
            Put_Line ("Texture_Management.Render Renderer_Ref is not created.");
            ...
            Put_Line ("Texture_Management.Render String_Texture is not initialized.");
      end if;
...
--                                    GL.Blending.One_Minus_Src_Alpha);
      ...
      if not GL.Objects.Textures.Is_Texture  (String_Texture.Raw_Id) then
         raise Texture_Management_Exception with
              "Texture_Management.Render_Text, String_Texture is invalid.";
      end if;
