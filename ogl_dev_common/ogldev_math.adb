...
package body Ogldev_Math is
    ...
    begin
        if Target /= (0.0, 0.0, 0.0)  then
            if Up /= (0.0, 0.0, 0.0) then
                N := Normalized (Target);
                ...
                if U /= (0.0, 0.0, 0.0) then
                    ...
                    raise Math_Exception with
                      "Ogldev_Math.Init_Camera_Transform attempted to normalize a zero length U vector.";
                end if;
            else
                raise Math_Exception with
                  "Ogldev_Math.Init_Camera_Transform attempted to normalize a zero length Up vector.";
            end if;
        else
            raise Math_Exception with
              "Ogldev_Math.Init_Camera_Transform attempted to normalize a zero length Target vector.";
        end if;
