...
package body Meshes_24N is
   ...
   begin
      ...
      Put_Line ("Meshes_24N.Init_From_Scene, initializing " &
                  File_Name);
   ...
   exception
      when others =>
         Put_Line ("An exception occurred in Meshes_24N.Init_From_Scene.");
         raise;
            ...
            if Result = Assimp_Types.API_Return_Success then
               if Ogldev_Texture.Init_Texture
                 (aTexture, GL.Low_Level.Enums.Texture_2D,
                  ...
                  Put_Line ("Meshes_24N.Init_Materials.Load_Textures loaded texture " &
                              GL.Types.UInt'Image (index) & " from "
                            & Dir & To_String (Path));
               else
                  Put_Line ("Meshes_24N.Init_Material.Load_Textures texture "
                            & Dir & To_String (Path) & " not found.");
               end if;
            else
               Put_Line ("Meshes_24N.Init_Material.Load_Textures Get_Texture result: " &
   ...
   exception
      when others =>
         Put_Line ("An exception occurred in Meshes_24N.Init_Material.API_Return_Success.");
         raise;
   end Init_Materials;
   ...
   begin
      ...
      if Source_Mesh.Faces.Is_Empty then
         Put_Line ("Meshes_24N.Init_Mesh, Source_Mesh.Faces is empty.");
      else
   ...
   exception
      when others =>
         Put_Line ("An exception occurred in Meshes_24N.Init_Mesh.");
         raise;
   end Init_Mesh;
   ...
   exception
      when others =>
         Put_Line ("An exception occurred in Meshes_24N.Init_Mesh_Entry.");
         raise;
   end Init_Mesh_Entry;
   ...
   exception
      when others =>
         Put_Line ("An exception occurred in Meshes_24N.Load_Mesh.");
         raise;
   end Load_Mesh;
                  ...
                  if aTexture.Texture_Object.Initialized then
                     ...
                     if not GL.Objects.Textures.Is_Texture
                       (GL.Objects.Raw_Id (GL.Objects.GL_Object (aTexture.Texture_Object))) then
                        Put_Line ("Meshes_24N.Render_Mesh, aTexture is invalid.");
                     end if;
                  else
                     Put_Line ("Meshes_24N.Render_Mesh, Texture_Object is not initialized.");
                  end if;
               else
                  Put_Line ("Meshes_24N.Render_Mesh, theMesh.Textures does not contain Material: " &
   ...
   exception
      when others =>
         Put_Line ("An exception occurred in Meshes_24N.Render.");
         raise;
   end Render;
