...
package body Prop_Renderer is
    ...
    function Props_In_Tiles_Size (U, V : Natural) return Natural is
    begin
        return Props_In_Tiles (U, V).Last_Index;
    ...
    procedure Render_Props_Around_Depth_Only (U, V, Tiles_Distance : Natural) is
        use Prop_Indices_Package;
        ...
        Left           : constant Integer :=
                           Maths.Max_Integer (0, V - Tiles_Distance) + 1;
        Right          : constant Integer := Maths.Min_Integer (Tiles_Manager.Max_Map_Cols - 1,
                                                                V + Tiles_Distance) + 1;
        Up             : constant Integer := Maths.Max_Integer (0, U - Tiles_Distance) + 1;
        Down           : constant Integer := Maths.Min_Integer (Tiles_Manager.Max_Map_Rows - 1,
                                                                U + Tiles_Distance) + 1;
        --  Diamond Bob
    ...
    begin
        ...
        for vi in Left .. Right loop
            for ui in Up .. Down loop
                if not Tiles_Manager.Is_Tile_Valid ((Int (ui), Int (vi))) then
                    raise Prop_Renderer_Exception with
                      "Prop_Renderer.Activate_Door_In_Tile, invalid tile indices: " &
                      Integer'Image (ui) & ", " & Integer'Image (vi);
                end if;
                Prop_Indices := Props_In_Tiles (ui, vi);
