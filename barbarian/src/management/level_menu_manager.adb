...
package body Level_Menu_Manager is
    ...
    Maps_List_File  : constant String := "src/save/maps.dat";
    ...
    begin
        if Maps.Is_Empty or else Selected_Map_ID > Maps.Last_Index then
            raise Level_Manager_Exception with
    ...
    procedure Write_Story_Names (Names_List : in out Levels_List) is
        use Levels_Package;
        Curs        : Cursor := Names_List.First;
        Output_File : File_Type;
    begin
        Open (Output_File, Out_File, Maps_List_File);
        if not Is_Open (Output_File) then
            raise Level_Manager_Exception with
              "Level_Menu_Manager.Write_Story_Names, error opening" &
              Maps_List_File & "for writing";
        end if;
        while Has_Element (Curs) loop
            Put_Line (Output_File, To_String (Element (Curs).Level_Name));
            next (Curs);
        end loop;
        Close (Output_File);
        Game_Utils.Game_Log ("Level_Menu_Manager.Write_Story_Names, " &
                              Maps_List_File & " written");
    end Write_Story_Names;
