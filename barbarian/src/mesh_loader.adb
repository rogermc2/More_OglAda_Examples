...
package body Mesh_Loader is
   ...
   function Get_Mesh (Mesh_ID : Integer; aMesh : in out Mesh) return Boolean is
      use Meshes_Package;
      Curs  : Cursor := Meshes.First;
      Found : Boolean := False;
   begin
   ...
   function Loaded_Mesh  (Index : Positive) return Mesh is
   begin
      return Meshes.Element (Index);
   end Loaded_Mesh;
   ...
   function Loaded_Mesh_Bounding_Radius
     (Mesh_ID : Positive; Radius : in out Float) return Boolean is
      theMesh : Mesh;
      Found   : Boolean := False;
   begin
      Found := Get_Mesh (Mesh_ID, theMesh);
      if Found then
         Radius := theMesh.Bounding_Radius;
      end if;
      return Found;

   end Loaded_Mesh_Bounding_Radius;
   ...
   begin
      Found := Get_Mesh (Mesh_ID, theMesh);
      if Found then
         VAO := theMesh.Shadow_VAO;
         if VAO = Null_Array_Object then
            raise Mesh_Loader_Exception with
              "Mesh_Loader.Loaded_Mesh_VAO, a Loaded Shadow_VAO VAO has not been initialised";
         end if;
      end if;
      return Found;

   end Loaded_Mesh_Shadow_VAO;
   ...
   begin
      Found := Get_Mesh (Mesh_ID, theMesh);
      if Found then
         Count := theMesh.Vp_Count;
      end if;
      return Found;
   end Loaded_Mesh_Vertex_Count;
   ...
   procedure Recurse_Animation_Tree
     (aMesh        : in out Mesh; Anim : in out Animation; Anim_Time : Float;
      My_Anim_Node : Integer; Parent_Matrix : Singles.Matrix4) is
   begin
      Null;
   end Recurse_Animation_Tree;
