...
package body Text is
    ...
    begin
        ...
        Text_To_VBO (theText, Size_In_Pixels, Points_Buffer, Tex_Coords_Buffer,
                     Point_Count, BR_X, BR_Y);
    ...
    procedure Text_To_VBO (theText : String; Scale_Px : Float;
        ...
        X_Pos              : Single;
        Y_Pos              : Single;
        String_Char        : String (1 .. 1);
    begin
        Br_X := 0.0;
        Br_Y := 0.0;
        for index in 1 .. Text_Length loop
            if theText (index) = Ascii.LF then
                Line_Offset := Line_Offset + 2.0 * Scale_Px_S / Font_VP_Height_S;
                Current_X := 0.0;
            end if;
            String_Char := theText (index .. index);
            Ascii_Code := Integer'Value (String_Char);
            Atlas_Col := (Ascii_Code - Integer'Value (" ")) mod Atlas_Cols;
            Atlas_Row := (Ascii_Code - Integer'Value (" ")) / Atlas_Cols;
