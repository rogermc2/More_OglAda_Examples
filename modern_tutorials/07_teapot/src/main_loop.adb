  ...
  --  ------------------------------------------------------------------------
   ...
   begin
      ...
      if Teapot_Mode = Teapot then
         ...
         GL.Attributes.Set_Vertex_Attrib_Pointer (Colour_Attribute, 3,
                                                  Single_Type, 0, 0);

         GL.Objects.Buffers.Element_Array_Buffer.Bind (Elements_Buffer);
         GL.Objects.Buffers.Draw_Elements (Triangles, Elements'Length,
                                           UInt_Type);
      else
         Draw_Control_Points;
      end if;
...
--                       GL.Types.Int'Image (index));
            GL.Objects.Buffers.Draw_Elements
              (Line_Loop, Teapot_Data.Order + 1, UShort_Type, Offset);
         end loop;
       end loop;
   ...
   begin
      ...
      if Result then
         ...
         Buffers.Create_CP_Vertex_Buffer
           (CP_Vertices_Buffer, Teapot_Data.Control_Points);
         Buffers.Create_CP_Colour_Buffer (CP_Colours_Buffer, CP_Colours);
         Buffers.Create_CP_Elements_Buffer (CP_Elements_Buffer, CP_Elements);  --  For debugging
      end if;
      return Result;
