...
package body Quaternions is
   ...
   function "*" (Left : Quaternion; Right : GL.Types.Singles.Vector3) return Quaternion is
      use GL;
      Q4 : constant Quaternion := (Real (Right (X)), Real (Right (Y)), Real (Right (Z)), 0.0);
   begin
      return Left * Q4;
   end "*";

   function "*" (Left : GL.Types.Singles.Vector3; Right : Quaternion) return Quaternion is
      use GL;
      Q4 : constant Quaternion := (Real (Left (X)), Real (Left (Y)), Real (Left (Z)), 0.0);
   begin
      return Q4 * Right;
   end "*";
